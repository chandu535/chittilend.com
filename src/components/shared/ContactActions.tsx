...
/** Shared by both variants, so the labelled pair and the bare glyphs cannot drift apart. */
...
interface ContactActionsProps {
  ...
  className?: string;
  /**
   * `full` is the labelled pair that sits at the foot of a card.
   *
   * `icons` is the same two actions as bare glyphs, for beside a name in a list row or a
   * table cell — places where a labelled button would be wider than the name it belongs
   * to. The tap target stays 44px either way; only the label goes.
   */
  variant?: 'full' | 'icons';
}
 ...
 * leads to a dead end.
 */
export function ContactActions({ mobile, name, className, variant = 'full' }: ContactActionsProps) {
    ...
    return (
      <div className={clsx('flex shrink-0 items-center gap-0.5', className)}>
        <a
          ...
          title={`${t('contact.call')} ${name}`}
          className="flex h-11 w-11 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 active:bg-slate-200"
        >
        ...
        <a
          ...
          title={`${t('contact.whatsapp')} ${name}`}
          className="flex h-11 w-11 items-center justify-center rounded-full text-emerald-600 transition-colors hover:bg-emerald-50 active:bg-emerald-100"
        >
  ...
  return (
    <div className={clsx('flex items-stretch gap-2', className)}>
      <a
        ...
        onClick={isolate}
        aria-label={`${t('contact.call')} ${name}`}
        className="flex min-h-11 flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-card text-[13px] font-semibold text-slate-600 transition-colors active:bg-slate-50"
      >
