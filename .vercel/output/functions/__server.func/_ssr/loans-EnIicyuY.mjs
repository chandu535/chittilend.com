import { c as createServerRpc, l as loans, b as borrowers, d as db, p as payments, a as capitalPoolLog } from "./index-BAKXOWjL.mjs";
...
import { e as eq, g as gte, l as lte, b as and, i as ilike, a as sql, d as desc, c as count } from "../_libs/drizzle-orm.mjs";
...
const listLoans = createServerFn({
  ...
  if (data.search) {
    ...
    const [items2, totalResult2] = await Promise.all([db.select({
      ...
      nextPayment: sql`(SELECT json_build_object('id',p.id::text,'installmentNumber',p.installment_number,'dueDate',p.due_date::text,'amountDue',p.amount_due::text,'amountPaid',p.amount_paid::text,'status',p.status::text) FROM payments p WHERE p.loan_id=${loans.id} AND p.status NOT IN ('paid','waived') ORDER BY p.installment_number ASC LIMIT 1)`,
      paidInstallments: sql`(SELECT COUNT(*) FROM payments p WHERE p.loan_id=${loans.id} AND p.status = 'paid')`,
      paidAmount: sql`COALESCE((SELECT SUM(p.amount_paid) FROM payments p WHERE p.loan_id=${loans.id} AND p.status IN ('paid', 'partial')), 0)`
    }).from(loans).innerJoin(borrowers, eq(loans.borrowerId, borrowers.id)).where(fullWhere).orderBy(sql`(SELECT CASE WHEN p.status = 'overdue' THEN 0 ELSE 1 END FROM payments p WHERE p.loan_id=${loans.id} AND p.status NOT IN ('paid','waived') ORDER BY p.installment_number ASC LIMIT 1) ASC NULLS LAST`, desc(loans.createdAt)).limit(data.limit).offset(offset), db.select({
      count: count()
  ...
  const [items, totalResult] = await Promise.all([db.select({
    ...
    paidInstallments: sql`(SELECT COUNT(*) FROM payments p WHERE p.loan_id=${loans.id} AND p.status = 'paid')`,
    paidAmount: sql`COALESCE((SELECT SUM(p.amount_paid) FROM payments p WHERE p.loan_id=${loans.id} AND p.status IN ('paid', 'partial')), 0)`
  }).from(loans).innerJoin(borrowers, eq(loans.borrowerId, borrowers.id)).where(where).orderBy(sql`(SELECT CASE WHEN p.status = 'overdue' THEN 0 ELSE 1 END FROM payments p WHERE p.loan_id=${loans.id} AND p.status NOT IN ('paid','waived') ORDER BY p.installment_number ASC LIMIT 1) ASC NULLS LAST`, desc(loans.createdAt)).limit(data.limit).offset(offset), db.select({
    count: count()
...
const changeStatus_createServerFn_handler = createServerRpc({
  id: "361d9805ac41dce6e3f105c867a61b961eed5b15f6fcfa1883da0be151707733",
  name: "changeStatus",
  filename: "src/server/functions/loans.ts"
}, (opts) => changeStatus.__executeServer(opts));
const changeStatus = createServerFn({
  method: "POST"
}).inputValidator((data) => {
  const d = data;
  if (!d.id) throw new Error("Loan ID is required");
  ...
  return d;
}).handler(changeStatus_createServerFn_handler, async ({
  data
}) => {
  const user = await getAuthenticatedUser();
  requireRole(user, ["admin", "manager"]);
...
export {
  changeStatus_createServerFn_handler,
